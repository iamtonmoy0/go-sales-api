...
func GetRevenues(c *fiber.Ctx) error {
	...
	for _, v := range order {
		if v.PaymentTypesId == 1 {
			...
			db.Where("id=?", 1).First(&paymentTypes)
			db.Where("payment_type_id=?", 1).First(&payment)
		...
		if v.PaymentTypesId == 2 {
			...
			db.Where("id=?", 2).First(&paymentTypes)
			db.Where("payment_type_id=?", 2).First(&payment)
		...
		if v.PaymentTypesId == 3 {
			...
			db.Where("id=?", 3).First(&paymentTypes)
			db.Where("payment_type_id=?", 3).First(&payment)
...
func GetSolds(c *fiber.Ctx) error {
	...
	for i := 0; i < len(duplicates); i++ {
		...
		prods := models.Product{}
		db.Where("id", duplicates[i]).Find(&prods)
